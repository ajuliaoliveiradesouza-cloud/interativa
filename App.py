...
historias = {
    ...
    "silencio": {
        ...
        "texto": "Voce chega em casa destruido e desconta a frustracao na sua familia. No dia seguinte, seu rendimento despenca e o clima fica insuportavel. O colega percebe que voce nao reagiu e comeca a montar em cima de voce, roubando novas ideias suas in reunioes, sabendo que voce aceita tudo calado.",
        "opcoes": {
...
if "opcoes" in no_atual and no_atual["opcoes"]:
    ...
    st.markdown("<h4 style='color: #fde047; margin-bottom: 15px;'>📝 CONSIDERACOES FINAIS</h4>", unsafe_allow_html=True)
    resposta_usuario = st.text_area("Diante de todo esse desfecho, o que voce faria de agora em diante ou qual licao voce tira disso?", placeholder="Escreva sua reflexao ou atitude final aqui...", height=120)
    
    if st.button("Gravar Resposta Final 🚀"):
        if resposta_usuario.strip() != "":
            st.session_state.respostas_usuarios.append(f"Cenario [{st.session_state.passo}]: {resposta_usuario}")
            st.success("Sua reflexao final foi salva com sucesso no painel!")
        else:
...
if st.sidebar.button("🔄 Reiniciar Historia"):
    st.session_state.passo = "inicio"
    st.rerun()
